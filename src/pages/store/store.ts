...
export class StorePage extends Component {
  ...
  constructor(parentNode: HTMLElement, model: Model) {
    ...
    model.subscribe((state) => {
      ...
      const minStock = state.minStock;
      const maxStock = state.maxStock;
      const productsAfterPriceAndStockFilter = productsAfterFilter.filter((item) => {
        if(item.price > maxPrice || item.price < minPrice || item.stock > maxStock || item.stock < minStock){
          return false
        }
        return true
      })
      ...
      if(sortingOrder ==='alph_asc'){
        productsAfterPriceAndStockFilter.sort((a:Product, b:Product) => {
      ...
      const itemsFound = productsAfterPriceAndStockFilter.length.toString();

      this.items.delete();
      this.foundWrapper?.delete()
      this.items = new Items(this.goodsWrapper.elem, productsAfterPriceAndStockFilter);
